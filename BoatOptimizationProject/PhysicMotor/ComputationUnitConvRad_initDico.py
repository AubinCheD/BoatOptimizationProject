#Units.angleConversion(beta,'\u00B0','rad')
...
class Computation ():            #rename en classe individu ?? + prendre 3 dicos : inputs / outputs / calcul
    ...
    def __init__ (self, inputDictionary, size):   #size = size of the vector of speeds
        ...
        self.inputDictionary = inputDictionary      #just a reference  // faire le test
    ...
    def calc_trim_eL(self, theta, roundingPrecision = 10, index = -1):      #ok
        ...
        if index == -1:
            ...
            lastIndex = index + 1
        for i in range(firstIndex, lastIndex):
            thetaTemp = Units.angleConversion(theta['value'], theta['unit'], '\u00B0')
            self.dictionary['trim_eL']['value'][i] = round(self.dictionary['trim']['value'][i] + 0.12*thetaTemp,  roundingPrecision)
    ...
    def calc_trim_eD(self, theta, roundingPrecision = 10, index = -1):      #ok
        ...
        for i in range(firstIndex, lastIndex):
            thetaTemp = Units.angleConversion(theta['value'], theta['unit'], '\u00B0')
            self.dictionary['trim_eD']['value'][i] = round(self.dictionary['trim']['value'][i] + 0.5*thetaTemp,  roundingPrecision)
    ...
    def calc_lambda(self, roundingPrecision = 10, index = -1):                    #ok         #division par zéro, à check
        ...
        for i in range(firstIndex, lastIndex):
            lam = 0
            while (lam < 10):
                temp = 0.0055/self.dictionary['Cv']['value'][i]/self.dictionary['Cv']['value'][i] * math.pow(lam, 2.5)
    ...
    def calc_beta_e(self, beta, theta, roundingPrecision = 10, index = -1):   #need lambda first        #ok
        ...
        if index == -1:
            ...
            firstIndex = index
            lastIndex = index + 1
        beta_temp =  Units.angleConversion(beta['value'],beta['unit'],'rad')
        theta_temp =  Units.angleConversion(theta['value'],theta['unit'],'rad')
        for i in range(firstIndex, lastIndex):
            betaTempDegree = Units.angleConversion(beta['value'], beta['unit'], '\u00B0')
            temp =  Units.angleConversion( math.atan(   math.tan(beta_temp) + self.dictionary['lambda']['value'][i]*math.tan(theta_temp)  ), 'rad', '\u00B0')
            self.dictionary['beta_e']['value'][i] = round( 0.5*(betaTempDegree + temp) ,  roundingPrecision)
    ...
    def calc_CL_beta_d(self, beta, roundingPrecision = 10, index = -1):     #ok
        ...
        for i in range(firstIndex, lastIndex):
            ...
            betaTemp = Units.angleConversion(beta['value'], beta['unit'], '\u00B0')
            temp = self.dictionary['CL0d']['value'][i] - 0.0065*betaTemp*pow(self.dictionary['CL0d']['value'][i],0.6)
    ...
    def calc_Vm(self, V, beta, roundingPrecision = 10, index = -1):         #ok    
        ...
        if index == -1:
            ...
            lastIndex = index + 1
        for i in range(firstIndex, lastIndex):
            temp = self.dictionary['lambda']['value'][i] * math.cos(Units.angleConversion(beta['value'], beta['unit'], 'rad')) * math.cos(Units.angleConversion(self.dictionary['trim']['value'][i],'\u00B0','rad'))
            '''print('temp = ' + str(temp))
    ...
    def calc_trimTabs_DELTA (self, V, B, beta, trimChord, trimSigma, trimDelta, roundingPrecision = 10, index = -1):            #ok
        ...
        if index == -1:
            ...
            lastIndex = index + 1
        for i in range(firstIndex, lastIndex):
            trimDeltaTemp = Units.angleConversion(trimDelta['value'], trimDelta['unit'], '\u00B0')
            self.dictionary['trimTabs_DELTA']['value'][i] = round(0.046*trimChord*trimSigma*trimDeltaTemp*B*0.5*Constants.constants['rhoWater_SI']['value']*V[i]*V[i] * math.cos(Units.angleConversion(beta['value'],beta['unit'],'rad')),  roundingPrecision)
    ...
    def calc_trimTabs_D_L (self, trimDelta, roundingPrecision = 10, index = -1):                  #ok
        ...
        for i in range(firstIndex, lastIndex):
            trimDeltaTemp = Units.angleConversion(trimDelta['value'], trimDelta['unit'], '\u00B0')
            self.dictionary['trimTabs_D_L']['value'][i] = round(0.0052*self.dictionary['trimTabs_DELTA']['value'][i]*(self.dictionary['trim']['value'][i] + trimDeltaTemp),  roundingPrecision)
    ...
    def calc_trimTabs_D (self, trimDelta, roundingPrecision = 10, index = -1):              #ok
        ...
        if index == -1:
            ...
            lastIndex = index + 1
        for i in range(firstIndex, lastIndex):
            temp = Units.angleConversion(trimDelta['value'], trimDelta['unit'], '\u00B0')  #need to make sure trimDelta is in degrees
            self.dictionary['trimTabs_D']['value'][i] = round(self.dictionary['trimTabs_Df']['value'][i]*math.cos(Units.angleConversion(self.dictionary['trim']['value'][i] + temp,'\u00B0','rad')) + self.dictionary['trimTabs_D_L']['value'][i],  roundingPrecision)
    ...
    def calc_trimTabs_M (self, LCG, VCG, B, trimChord, trimSigma, trimDelta, roundingPrecision = 10, index = -1):       #ok
        ...
        if index == -1:
            ...
            lastIndex = index + 1
        for i in range(firstIndex, lastIndex):
            temp = LCG - 0.6*B - trimChord*(1-trimSigma-math.cos(Units.angleConversion(trimDelta['value'],trimDelta['unit'],'rad')))
    ...
    def calc_shaft_D (self, V, shaftDiameter, shaftLength, e, roundingPrecision = 10, index = -1):          #ok
        ...
        for i in range(firstIndex, lastIndex):
            ...
            temp = temp *(1.1 * math.pow(math.sin(Units.angleConversion(e['value'],e['unit'],'rad')), 3) + math.pi*self.dictionary['shaft_Cf_L']['value'][i])
    ...
    def calc_shaft_N (self, V, shaftDiameter, shaftLength, e, roundingPrecision = 10, index = -1):          #ok
        ...
        for i in range(firstIndex, lastIndex):
            ...
            temp = temp * math.pow(math.sin(Units.angleConversion(e['value'],e['unit'],'rad')), 2) * math.cos(Units.angleConversion(e['value'],e['unit'],'rad'))
    ...
    def calc_f_air(self, LCG, VCG, roundingPrecision = 10, index = -1):        #need h and Zprim       #a rajouter              #ok
        ...
        for i in range(firstIndex, lastIndex):
            ...
            #print(VCG * math.cos(math.radians(self.dictionary['trim']['value'][i])))
            temp = temp + LCG * math.sin(Units.angleConversion(self.dictionary['trim']['value'][i],'\u00B0','rad'))
            #print( LCG * math.sin(math.radians(self.dictionary['trim']['value'][i])))
    ...
    def calc_T(self, e, roundingPrecision = 10, index = -1):                #ok
        ...
        if index == -1:
            ...
            lastIndex = index + 1
        for i in range(firstIndex, lastIndex):
            temp = Units.angleConversion(e['value'], e['unit'], '\u00B0')
            self.dictionary['T']['value'][i] = round(self.dictionary['Fh']['value'][i]/math.cos(Units.angleConversion(self.dictionary['trim_eD']['value'][i] + temp,'\u00B0','rad')),  roundingPrecision)
    ...
    def calc_Nt(self, e, roundingPrecision = 10, index = -1):              #ok
        ...
        for i in range(firstIndex, lastIndex):
            temp = Units.angleConversion(e['value'], e['unit'], '\u00B0')
            self.dictionary['Nt']['value'][i] = round(self.dictionary['Fv']['value'][i]*math.cos(Units.angleConversion(self.dictionary['trim_eD']['value'][i] + temp,'\u00B0','rad')) + self.dictionary['Fh']['value'][i]*math.sin(Units.angleConversion(self.dictionary['trim_eD']['value'][i] + temp,'\u00B0','rad')),  roundingPrecision)
    ...
    def calc_a(self, VCG, B, beta, roundingPrecision = 10, index = -1):            #ok
        ...
        if index == -1:
            ...
            lastIndex = index + 1
        for i in range(firstIndex, lastIndex):
            self.dictionary['a']['value'][i] = round(VCG - 0.25*B*math.tan(Units.angleConversion(beta['value'],beta['unit'],'rad')),  roundingPrecision)
    ...
    def calc_lambda_k(self, roundingPrecision = 10, index = -1):                    #ok
        ...
        for i in range(firstIndex, lastIndex):
            ...
            temp2 = math.tan(Units.angleConversion(self.dictionary['beta_e']['value'][i],'\u00B0','rad')) / 2
            temp2 = temp2 / math.tan(Units.angleConversion(self.dictionary['trim']['value'][i],'\u00B0','rad')) 
            #print('3) = ' + str(temp2))
            ...
            #print('5) = ' + str(math.tan(math.radians(self.dictionary['beta_e']['value'][i]))))
            #print('6) = ' + str(math.tan(math.radians(self.dictionary['trim']['value'][i]))))
...
## for every angle, give the angle dictionary to the function and not just the value
...
## some calc are constant in time
    def calcAllConstants(self, roundingPrecision = 10, index = -1):
        ...
        self.calc_CL0(self.inputDictionary['beta']['value'], roundingPrecision, index)   #ok    #constant
        
        self.calc_trimTabs_DELTA(self.inputDictionary['V']['SI'], self.inputDictionary['B']['value'], self.inputDictionary['beta'], self.inputDictionary['TrimTab_chord']['value'], self.inputDictionary['TrimTab_sigma']['value'], self.inputDictionary['TrimTab_delta'], roundingPrecision, index)      #ok        #constant
        ...
        self.calc_shaft_D(self.inputDictionary['V']['SI'], self.inputDictionary['Shaft_diameter']['value'], self.inputDictionary['Shaft_length']['value'], self.inputDictionary['e'], roundingPrecision, index) #need shaft_Cf    #constant        #ok
        self.calc_shaft_N(self.inputDictionary['V']['SI'], self.inputDictionary['Shaft_diameter']['value'], self.inputDictionary['Shaft_length']['value'], self.inputDictionary['e'], roundingPrecision, index) #constant          #ok
    ...
    def calcAll(self, roundingPrecision = 10, index = -1):        ## séparer en 2 fonctions -> certaines valeurs n'ont pas besoin d'être recalculées
        ...
        #self.calc_Ass(self.inputDictionary['Hss']['value'], self.inputDictionary['Bss']['value'], index)       #constante
        
        #self.calc_Fn_vol(self.inputDictionary['V']['SI'], self.inputDictionary['DELTA']['value'], index)        #constante
        
        #self.calc_Cv(self.inputDictionary['V']['SI'], self.inputDictionary['B']['value'], index)                #constante
        #self.calc_CLb(self.inputDictionary['V']['SI'], self.inputDictionary['B']['value'], self.inputDictionary['DELTA']['value'], index)      #constante
        
        #self.calc_CL0(self.inputDictionary['beta']['value'], index)       #constant
        
        #self.calc_trim()
        self.calc_trim_eL(self.inputDictionary['theta'], roundingPrecision, index)     #ok
        self.calc_trim_eD(self.inputDictionary['theta'], roundingPrecision, index)     #ok

        self.calc_lambda(roundingPrecision, index)  
        
        self.calc_beta_e(self.inputDictionary['beta'], self.inputDictionary['theta'], roundingPrecision, index)  #need lambda         #ok
        
        self.calc_CL0d(roundingPrecision, index) #need lambda et trim_eL        #ok
        self.calc_CL_beta_d(self.inputDictionary['beta'], roundingPrecision, index) #need CL0d     #ok
        self.calc_Vm(self.inputDictionary['V']['SI'], self.inputDictionary['beta'], roundingPrecision, index) #need lambda         #ok
        ...
        #tabs
        #self.calc_trimTabs_DELTA(self.inputDictionary['V']['SI'], self.inputDictionary['B']['value'], self.inputDictionary['beta']['value'], self.inputDictionary['TrimTab_chord']['value'], self.inputDictionary['TrimTab_sigma']['value'], self.inputDictionary['TrimTab_delta']['value'], index)         #constante
        ...
        self.calc_trimTabs_D_L(self.inputDictionary['TrimTab_delta'], roundingPrecision, index) #need trimTabs_DELTA, trim         #ok
        self.calc_trimTabs_D(self.inputDictionary['TrimTab_delta'], roundingPrecision, index) #need trimTabs_Df, trimTabs_D_L, trim        #ok
        self.calc_trimTabs_M(self.inputDictionary['LCG']['value'], self.inputDictionary['VCG']['value'],self.inputDictionary['B']['value'], self.inputDictionary['TrimTab_chord']['value'], self.inputDictionary['TrimTab_sigma']['value'], self.inputDictionary['TrimTab_delta'], roundingPrecision, index) #need trimTabs_DELTA, trimTabs_D              #ok
        #self.calc_trimTabs_H(self.inputDictionary['TrimTab_chord']['value'], index) #need trimTabs_DELTA   -> constant
        
        #shaft
        #self.calc_shaft_Re_D(self.inputDictionary['V']['SI'], self.inputDictionary['Shaft_diameter']['value'], index) #contant
        #self.calc_shaft_Re_L(self.inputDictionary['V']['SI'], self.inputDictionary['Shaft_length']['value'], index) #constant
        #self.calc_shaft_Cf_L(self.inputDictionary['Shaft_length']['value'], index) #need shaft_Re_L    #constant
        #self.calc_shaft_D(self.inputDictionary['V']['SI'], self.inputDictionary['Shaft_diameter']['value'], self.inputDictionary['Shaft_length']['value'], self.inputDictionary['e']['value'], index) #need shaft_Cf    #constant
        #self.calc_shaft_N(self.inputDictionary['V']['SI'], self.inputDictionary['Shaft_diameter']['value'], self.inputDictionary['Shaft_length']['value'], self.inputDictionary['e']['value'], index) #constant
        #self.calc_shaft_f(self.inputDictionary['VCG']['value'], self.inputDictionary['Shaft_yc']['value'], index) #constant, should not be a tab
        #self.calc_shaft_e(self.inputDictionary['LCG']['value'], self.inputDictionary['Shaft_xc']['value'], index)   #constant, should not be a tab
        ...
        #self.calc_strut_Re_c(self.inputDictionary['V']['SI'], self.inputDictionary['Strut_chord']['value'], index) #constant
        #self.calc_strut_CD(self.inputDictionary['Strut_chord']['value'], self.inputDictionary['Strut_thickness']['value'], index) #constant normally
        #self.calc_strut_D(self.inputDictionary['V']['SI'], self.inputDictionary['Strut_area']['value'], index) #constant normally
        #self.calc_strut_f(self.inputDictionary['VCG']['value'], self.inputDictionary['Strut_yc']['value'], index) #constant
        #self.calc_strut_e(self.inputDictionary['LCG']['value'], self.inputDictionary['Strut_xc']['value'], index) #constant
        #self.calc_strut_M(index) #constant normally
        
        #rudder      #faire des tests avec d'autres valeurs; rajouter les need
        #self.calc_rudder_Re_c(self.inputDictionary['V']['SI'], self.inputDictionary['Rudder_chord']['value'], index) #constant
        #self.calc_rudder_CD(self.inputDictionary['Rudder_chord']['value'], self.inputDictionary['Rudder_thickness']['value'], index) #constant normally
        #self.calc_rudder_D(self.inputDictionary['V']['SI'], self.inputDictionary['Rudder_area']['value'], index) #constant normally
        #self.calc_rudder_f(self.inputDictionary['VCG']['value'], self.inputDictionary['Rudder_yc']['value'], index) #constant
        #self.calc_rudder_e(self.inputDictionary['LCG']['value'], self.inputDictionary['Rudder_xc']['value'], index) #constant
        #self.calc_rudder_M(index) #constant normally
        
        #resistance to air
        #self.calc_Zprim(self.inputDictionary['Z']['value'], self.inputDictionary['LOA']['value'], index)      #need h first
        #self.calc_A_romp(self.inputDictionary['Bmax']['value'], index) #need Zprim
        #self.calc_A_voorkant(index) #need A_romp
        #self.calc_D_air(self.inputDictionary['V']['SI'], index) #need A_voorkant
        #self.calc_f_air(index)
        #self.calc_M_air(index)
        
        #total resistance
        #self.calc_Fh(self.inputDictionary['propellersNumber']['value'], index)                                                     
        #self.calc_Fv(self.inputDictionary['propellersNumber']['value'], self.inputDictionary['DELTA']['value'], index)            
        
        #self.calc_T(self.inputDictionary['e']['value'], index)
        #self.calc_Nt(self.inputDictionary['e']['value'], index)
        ...
        self.calc_a(self.inputDictionary['VCG']['value'], self.inputDictionary['B']['value'], self.inputDictionary['beta'], roundingPrecision, index) #constant        #ok
        ...
        self.calc_T(self.inputDictionary['e'], roundingPrecision, index) #need Fh, trim_eD             #ok
        self.calc_Nt(self.inputDictionary['e'], roundingPrecision, index) #need Fh, Fv, trim_eD        #ok
